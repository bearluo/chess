...
StateMachine.STYPE_WAIT = 1;
StateMachine.STYPE_WAIT = 2;
...
StateMachine.run = function(self)
	...
	if self.m_lastState and self.m_style and self.m_styleFuncMap[self.m_style] then	
		self.m_styleFuncMap[self.m_style](newStateObj,self.m_lastState.stateObj,self,StateMachine.onSwitchEnd,self.m_changeStyle);
	else
...
StateMachine.leftInAnim = function(newStateObj,lastStateObj,switchObj,switchFn)
    local lastRoot = lastStateObj.m_controller.m_view.m_root;
    local newRoot = newStateObj.m_controller.m_view.m_root;
...
StateMachine.customWaitAnim = function(newStateObj,lastStateObj,switchObj,switchFn,changeStyle)
    ...
    local lw,lh = lastStateObj.m_controller.m_view:getSize();
    local nw,nh = newStateObj.m_controller.m_view:getSize();
    ...
    if newStateObj.m_controller.m_view.setAnimItemEnVisible then
        newStateObj.m_controller.m_view.setAnimItemEnVisible(newStateObj.m_controller.m_view,false);  
    end
    if newStateObj.m_controller.m_view.resumeAnimStart then
        newStateObj.m_controller.m_view.resumeAnimStart(newStateObj.m_controller.m_view,lastStateObj,obj);  
    end
    if lastStateObj.m_controller.m_view.pauseAnimStart then
        lastStateObj.m_controller.m_view.pauseAnimStart(lastStateObj.m_controller.m_view,newStateObj,obj);
    end
