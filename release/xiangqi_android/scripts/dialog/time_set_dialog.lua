...
TimeSetDialog.initDefaultTime = function(self)
	...
	local sumtime = UserInfo.getInstance():getSumTime();
	self.m_time1_min = math.floor(sumtime/60);
	self.m_time1_sec = sumtime%60;

	local steptime = UserInfo.getInstance():getStepTime();
	self.m_time2_min = math.floor(steptime/60);
	self.m_time2_sec = steptime%60;

	local readtime = UserInfo.getInstance():getReadTime();
...
TimeSetDialog.cancel = function(self)
    ...
    self.m_room.m_controller:sendSocketMsg(CLIENT_MSG_LEAVE,nil, nil,1);
--	local processer = ProcessFactory.getInstance():getProcesser(CLIENT_MSG_LEAVE);   --获取命令字
--	local ret = processer:doRequest(self.m_room);
