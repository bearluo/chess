...
ReplayRoomController.onDownLoadImage = function(self , flag,json_data)
	if not json_data then
		return;
	end

	local info = json.analyzeJsonNode(data);
	self:updateView(ReplayRoomScene.s_cmds.updataUserIcon,info.ImageName,tonumber(info.what));
end
-------------------------------- http ------------------------------------
...
ReplayRoomController.s_nativeEventFuncMap = {
    [kCacheImageManager]               = ReplayRoomController.onDownLoadImage;
};
