...
local function createCircleScanShader()
    program_create("circleScan");
    ...
    local fsCircleScan = [=[
        ...
        {   
            ...
            float color = -displayClickWiseArea * sign(angle - atan(uv.x, uv.y));

            gl_FragColor = vec4(color*colorbg.xyz* colorbg.a,color * colorbg.a) * u_color;
        }
    ]=];

    program_set_shader_source("circleScan", vsCircleScan, fsCircleScan);

    program_add_parameter("circleScan", PROGRAM_DATA_TYPE_UNIFORM, 44, "u_mvp_matrix");
    program_add_parameter("circleScan", PROGRAM_DATA_TYPE_ATTRIB, 3, "a_position");
    program_add_parameter("circleScan", PROGRAM_DATA_TYPE_TEXTURE, 0, "texture0");
    program_add_parameter("circleScan", PROGRAM_DATA_TYPE_ATTRIB, 2, "a_tex_coord");
    program_add_parameter("circleScan", PROGRAM_DATA_TYPE_UNIFORM, 4, "u_color");
    program_add_parameter("circleScan", PROGRAM_DATA_TYPE_INDEX, 2, "index");
    program_add_parameter("circleScan", PROGRAM_DATA_TYPE_UNIFORM, 1, "progress");
    program_add_parameter("circleScan", PROGRAM_DATA_TYPE_UNIFORM, 1, "displayClickWiseArea");
    program_add_parameter("circleScan", PROGRAM_DATA_TYPE_UNIFORM, 4, "offsetMatrix");
end
...
local function createHoloShader()

    program_create("holoShader");
    ...
    local fsHolo = [=[
            ...
            {
                vec4 colorBack = texture2D(texture0, vtexcoord);
                vec2 scaleTc = (vtexcoord * 2.0 - 1.0) * vec2(0.99,0.98);
                vec4 color = texture2D(texture3, scaleTc * 0.5 + 0.5);

                vec2 p = 2.0 * vtexcoord - 1.0;
                float a = atan(p.x,p.y);
                float r = length(p)*0.75;
                vec2 uv = vec2(a/6.283185307,r);

                float xCol = (uv.x - (time * 1.0 / 3.0)) * 3.0;
                xCol = mod(xCol, 3.0);
                ...
                float gray = dot(color.rgb, vec3(0.299, 0.587, 0.114));
                float newColor = gray * 3.0;

                gl_FragColor = (vec4(newColor* color.a,newColor* color.a,newColor * color.a,color.a)+0.5) * vec4(horColour * color.a ,color.a)* u_color* time * 3.0*(1.0 - colorBack.a * 0.99)  + colorBack;
            }
        ]=];

    program_set_shader_source("holoShader", vsHolo, fsHolo);

    program_add_parameter("holoShader", PROGRAM_DATA_TYPE_UNIFORM, 44, "u_mvp_matrix");
    program_add_parameter("holoShader", PROGRAM_DATA_TYPE_ATTRIB, 3, "a_position");
    program_add_parameter("holoShader", PROGRAM_DATA_TYPE_TEXTURE, 0, "texture0");
    program_add_parameter("holoShader", PROGRAM_DATA_TYPE_TEXTURE, 0, "texture3");
    program_add_parameter("holoShader", PROGRAM_DATA_TYPE_ATTRIB, 2, "a_tex_coord");
    program_add_parameter("holoShader", PROGRAM_DATA_TYPE_UNIFORM, 4, "u_color");
    program_add_parameter("holoShader", PROGRAM_DATA_TYPE_INDEX, 2, "index");
    program_add_parameter("holoShader", PROGRAM_DATA_TYPE_UNIFORM, 1, "time");
    program_add_parameter("holoShader", PROGRAM_DATA_TYPE_UNIFORM, 3, "holoColor");
end
