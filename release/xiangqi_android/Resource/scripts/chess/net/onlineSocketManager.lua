...
require(NET_PATH.."onlineSocketManagerProcesser")
...
OnlineSocketManager.ctor = function(self,socketType, gameId, deviceType, ver, subVer)
    ...
    self:addSocketProcesser(OnlineSocketManagerProcesser.getInstance())
...
OnlineSocketManager.openSocket = function(self)
 	if self:isSocketOpen() or self:isSocketOpening() or self.m_kickUser or not UserInfo.getInstance():isLogin() then
		return;
	end
    ...
    -- socket 的超时计时器会莫名未响应 这边做双保险 保证socket 超时 会置self.m_isSocketOpening 为 true
    delete(self.m_opening_anim);
    self.m_opening_anim = AnimFactory.createAnimInt(kAnimNormal, 0, 1, 10000, -1);
