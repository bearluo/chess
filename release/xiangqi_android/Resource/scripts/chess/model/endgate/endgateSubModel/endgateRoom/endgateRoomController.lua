...
--    local url = "http://h5.oa.com/chess/?type=1&id="..id;--测试服
    local urlStr = UserInfo.getInstance():getShareEndgateUrl()or "https://cnchess.17c.cn/h5/";--正式服
    local url = require("libs/url");
    local u = url.parse(urlStr);
    local params = {}
    params.type = 1
    params.id = id
    u:addQuery(params);
...
EndgateRoomController.onLoadEndingBoardByAndroid = function(self,status,data)
    if not status or data:get_value() == nil then
...
EndgateRoomController.exchangePropCallBack = function(self,isSuccess,message)
	...
	if status == -2 then
        if payRecommend and payRecommend.buyProp then
            ...
            local payData = {}
            payData.pay_scene = PayUtil.s_pay_scene.booth_prop
