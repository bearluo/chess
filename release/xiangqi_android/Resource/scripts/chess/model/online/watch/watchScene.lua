...
WatchScene.setSelectClick = function(self,flag)
    ...
    if flag then
        self.m_friends_btn:setFile("online/watch/friend_watch_nor.png");
        self.m_master_btn:setFile("online/watch/master_watch_room_chose.png");
    else
        self.m_friends_btn:setFile("online/watch/friend_watch_chose.png");
...
-- 更新魅力和我的关注观战列表
WatchScene.onUpdataCharmWatchGame = function(self,data)
    print_string("WatchScene.onUpdatafWatchGame" );
    self:resetWatchList();
    self.m_watch_list_loading_text:setVisible(false);
...
--    end
    ...
    if data and table.maxn(data) > 0 then
        self.m_watch_list_data = data;
        self.m_watch_adapter = nil;
		self.m_watch_adapter = new(WatchGameItemCacheAdapter,FriendWatchItem,self.m_watch_list_data);--WatchGameItem
        self.m_watch_adapter:setWatchSceneHandler(self);
        self.m_watch_list:setAdapter(self.m_watch_adapter);
        self.m_watch_list:setVisible(true);
    end
...
WatchScene.onUpdataRoomLevel = function(self,data)
    if not data then return end
    local children = self.m_watch_list:getChildren()
    for k,v in pairs(children) do
        if v and type(v.updataWatchRoomLevel) == "function" then
            v:updataWatchRoomLevel(data)
        end
    end
    self.mRoomLevelData = data
end
...
WatchScene.s_cmdConfig = 
{
    ...
    [WatchScene.s_cmds.updata_charm_watch_game]		    = WatchScene.onUpdataCharmWatchGame;
    [WatchScene.s_cmds.updata_tid_status]		        = WatchScene.onUpdataRoomLevel;
...
----------我的关注item--------
require(VIEW_PATH .. "friend_watch_item")

FriendWatchItem = class(Node);

function FriendWatchItem.ctor(self,room)
	self.m_data = room;
    self.m_obtid = room.tid;
    self.m_root_view = SceneLoader.load(friend_watch_item);
    self.m_root_view:setAlign(kAlignCenter);
    self:addChild(self.m_root_view);
    local w,h = self.m_root_view:getSize();
    self:setSize(w,h*1.1);
    ...
    self.icon = self.m_btn:getChildByName("head_bg"):getChildByName("head_mask")
    self.level = self.m_btn:getChildByName("head_bg"):getChildByName("level")
    self.name = self.m_btn:getChildByName("name")
    self.watch_text = self.m_btn:getChildByName("watch_num_bg"):getChildByName("text")
    self.watch_num = self.m_btn:getChildByName("watch_num_bg"):getChildByName("num")
    self.room_tip = self.m_btn:getChildByName("tip_room")

    self:updataItem()
end

function FriendWatchItem.setUserInfo(self)
    self.setUser = false
...
function FriendWatchItem.updataItem(self)
    self:setHeadIcon()
    ...
    self.watch_num:setText((self.m_data.ob_num or 0));
    local w,h = self.watch_num:getSize()
    self.watch_text:setPos(12 + w,0)
end

function FriendWatchItem.updataRoomTips(self,data)
    if not data then return end

end

function FriendWatchItem.setHeadIcon(self)
...
function FriendWatchItem.setWatchSceneHandler(self,handler)
    self.m_handler = handler;
...
function FriendWatchItem.onClick(self)
    if self.m_handler then
        WatchScene.onWatchListItemClick(self.m_handler,self);
    end
end

function FriendWatchItem.getData(self)
	return self.m_data;
end

function FriendWatchItem.updataWatchRoomLevel(self,data)
...
function FriendWatchItem.dtor(self)
