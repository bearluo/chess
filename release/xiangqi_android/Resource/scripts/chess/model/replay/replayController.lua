...
-- 获得我的收藏
-- ReplayScene有2种触发类型，带时间是筛选使用，不带时间主界面使用
ReplayController.onGetMySaveChess = function(self,startIndex,num,startTime,endTime)
    local post_data = {};
    post_data.mid = UserInfo.getInstance():getUid();
...
-- 获得棋友动态
ReplayController.onGetSuggestChess = function(self,startIndex,num,startTime,endTime)
    local post_data = {};
    post_data.mid = UserInfo.getInstance():getUid();
    if startTime and endTime then
        post_data.start_time = startTime;
...
ReplayController.onGetMyChessCallBack = function(self, flag, message)
    ...
    if self.m_mysave_chess_bytime then
        self:updateView(ReplayScene.s_cmds.get_mychess_bytime,data);
    else
...
--            ChessToastManager.getInstance():showSingle(message.error:get_value(),2000);
    ...
    if self.m_suggest_chess_bytime then
        self:updateView(ReplayScene.s_cmds.get_suggest_bytime,data);
    else
