...
require(DIALOG_PATH .. "notice_view_mail_dialog2");
require(DIALOG_PATH .. "match_prize_dialog");
...
UserInfoSceneNoticeMsgListItem.ctor = function(self,data,handler)
    ...
    elseif data.mail_type == kMailUser then
         mailType = "用户消息";
    elseif data.mail_type == kMailMatch then
         mailType = "比赛消息";
    end
    ...
    elseif finger_action == kFingerUp then
        if drawing_id_first == drawing_id_current then
            ...
            if self.isOnTouchClick then
                ...
                if self.data.tpl_type and tonumber(self.data.tpl_type) == 4 then
                    if not self.m_match_dialog then
                        self.m_match_dialog = new(MatchPrizeDialog,self.data);
                    end
                    self.m_match_dialog:show();                    
                elseif self.data.tpl_type == kMailTplGoodPay or self.data.tpl_type == kMailTplGoodExchange then
                    if not self.notice_dialog2 then
                        self.notice_dialog2 = new(NoticeViewMailDialog2,self.data);
                    end
                    self.notice_dialog2:show(self.data);     
                elseif self.data.tpl_type == kMailTpModifyMnick then
                    UserInfo.getInstance():setModifyMnick(true);
