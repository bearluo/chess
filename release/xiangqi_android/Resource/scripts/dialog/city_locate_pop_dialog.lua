...
CityLocatePopDialog.ctor = function(self)
    ...
    self:setShieldClick(self,self.hide);
    ...
    -- locate_fail
    self.m_locate_fail_view = self.m_bg:getChildByName("locate_fail");
    self.m_locate_btn = self.m_locate_fail_view:getChildByName("locate");
...
CityLocatePopDialog.show = function(self,room,showLocateView)
    self.m_room = room;
    self:loadList();
    ...
    self.m_locate_fail_view:setVisible(showLocateView);
...
CityLocatePopDialog.hide = function(self)
    self:setVisible(false);
    self.super.dismiss(self,false);
    for i = 1,3 do 
        if not self.m_bg:checkAddProp(i) then
...
CityLocatePopDialog.setLocateBtnEvent = function(self,func,obj)
    self.m_locate_btn:setOnClick(self,function() 
        self:hide();
        func(obj);
    end);
end;
...
CityLocatePopDialog.onSaveUserInfoCityResponse = function(self,isSuccess,message)
    if isSuccess then
        local location = json.analyzeJsonNode(message.data);
        local aUser = location.aUser;
        UserInfo.getInstance():setProvinceCode(aUser.province_code or 0);
        UserInfo.getInstance():setProvinceName(aUser.province_name or "");
        UserInfo.getInstance():setCityCode(aUser.city_code or 0);
        UserInfo.getInstance():setCityName(aUser.province_name or "");

        GameCacheData.getInstance():saveString(GameCacheData.LOCATE_CITY_NAME,UserInfo.getInstance():getProvinceName());
        GameCacheData.getInstance():saveInt(GameCacheData.LOCATE_CITY_CODE,UserInfo.getInstance():getCityCode());

        if self.m_room and self.m_room.m_chat_room_tittle then
            self.m_room.m_chat_room_tittle:setText(UserInfo.getInstance():getProvinceName() .. "棋友聊天室");
        elseif self.m_room and self.m_room.setCityName then
            self.m_room:setCityName(UserInfo.getInstance():getProvinceName());
        end
