...
CreateRoomDialog.ctor = function(self,x,y,w,h,room)
    ...
    self.m_defaultRoomName = string.subutf8(UserInfo.getInstance():getName(),1,6).."的房间";
    ...
    if config then
        self.m_create_room_view_bg:getChildByName("input_dizhu_bg"):getChildByName("edit"):setText( string.format("%d金币",config.money))
    end
    self:initData();
...
CreateRoomDialog.sure = function(self)
    ...
    if string.lenutf8(roomnameStr) > 9 then 
        ChessToastManager.getInstance():showSingle("房间名字最长可输入9个字")
        return 
    end
