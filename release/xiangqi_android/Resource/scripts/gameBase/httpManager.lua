...
HttpManager.execute = function(self,command,data,tip,canCancel,level,isNeedMask)
    ...
    elseif config[HttpConfigContants.URL] ~= "https://feedback.kx88.net/api/api.php" then
        if httpType == kHttpPost then 
    ...
    HttpLoadDialogManager.getInstance():push(command,tip,level,canCancel,isNeedMask)
...
HttpManager.execute2 = function(self,command,data,callbackFunc)
	if not HttpManager.checkCommand(self,command) then
		return false;
	end
	...
	local httpRequest = new(Http,httpType,kHttpReserved,url)
	httpRequest:setEvent(callbackFunc, self.onResponse2);
	httpRequest:setTimeout(self.m_timeout,self.m_timeout);
...
HttpManager.onResponse2 = function(callbackFunc , httpRequest)
	...
	if errorCode ==HttpErrorType.SUCCESSED then
        if type(callbackFunc) == "function" then
            -- 保护模式执行
            if kDebug then
                callbackFunc(true,resultStr,httpRequest)
            else
                pcall(function ()callbackFunc(true,resultStr,httpRequest) end)
            end
        end
    else
        if type(callbackFunc) == "function" then
            -- 保护模式执行
            if kDebug then
                callbackFunc(false,resultStr,httpRequest)
            else
                pcall(function ()callbackFunc(false,resultStr,httpRequest) end)
            end
        end
	end
end
...
HttpLoadDialogManager.push = function(self,cmd,str,level,canCancel,isNeedMask)
    local tbl = {};
    tbl.cmd = cmd;
    tbl.str = str;
    ...
    self:changeLoadingDialog(isNeedMask);
end

HttpLoadDialogManager.changeLoadingDialog = function(self,isNeedMask)
    local newCmd = nil
...
--        if isNeedMask == nil then isNeedMask = true end
        isNeedMask = (isNeedMask == nil and true or false)
        self.m_loadingDialog:setNeedMask(isNeedMask)
